...
function Start-MainMenu {
    ...
    Write-Host "!! Enter <Q> to quit or click the windows [ X ] button close this window !!`n"

    # display the main menu options
    Write-Host "<1> Cleanup Temp & Junk Files"
    Write-Host "    <R> Empty all Recycle Bins (all users)"
    Write-Host "    <U> Clean Windows Update Download Cache"
    Write-Host "<2> . . ."
    Write-Host "<3> . . ."
    Write-Host "<4> . . ."
    Write-Host "<5> . . ."
    ...
    if ($menuselect -eq "Q") {
        Clear-Host
        Write-Host "Quitting!"
        [void](Read-Host "Press [RETURN] to continue...")
        Clear-Host
        Exit
    } else {
        Clear-Host
